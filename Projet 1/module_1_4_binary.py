...
for tokens in df_HQ['Tokens']: #comptage des tokens dans HQ
    ...
    for token in unique_tokens: #on ne compte pas les occurences ici
        if token in vocab_set: 
            tokens_HQ[token] = tokens_HQ.get(token, 0) + 1

for tokens in df_LQ['Tokens']: #comptage des tokens dans LQ
    unique_tokens = set(tokens)
    for token in unique_tokens: #on ne compte pas les occurences ici
        if token in vocab_set:
            tokens_LQ[token] = tokens_LQ.get(token, 0) + 1
...
for tokens in df_test['Tokens']: #nettoyage du test set pour Naive Bayes
    temp_list = []
    unique_tokens = set(tokens)
    for token in unique_tokens: #même chose qu'en haut mais on n'ajoute pas les balises <s> et </s>
