...
#---1.4--- Negative Tokens Preprocessing
...
def apply_negative(tokens):
    new_tokens = []
    negation = False

    for token in tokens:
        if token in negatives:
            negation = True #si le token est une négation, on met le booléen negation à true pour la suite
            new_tokens.append(token)
            continue
        if token in punctuations:
            negation = False #si le token est une ponctuation, fin de la phrase on stop la négation si il y en avait une avant
            new_tokens.append(token)
        
        if negation: #si négation, on ajoute un _NOT devant le token
            new_tokens.append(f"{token}_NOT") #https://www.w3schools.com/python/python_string_formatting.asp
        else:
            new_tokens.append(token) #si pas de négation on ajoute bêtement le token
    return new_tokens

df_corpus['Tokens'] = df_corpus['Tokens'].apply(apply_negative) #on applique la fonction sur df_corpus['Tokens'] et df_test['Tokens']
df_test['Tokens'] = df_test['Tokens'].apply(apply_negative) #on applique la fonction sur df_corpus['Tokens'] et df_test['Tokens']
